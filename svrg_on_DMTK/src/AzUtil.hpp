...
public: 
  ...
  int readBytes(AzByte *buff, int buff_len) {
    size_t my_len = fread(buff, 1, buff_len, fp); 
    int len = Az64::to_int(my_len); 
    if (len != buff_len && !feof(fp)) throw new AzException(AzFileIOError, "AzFile::readBytes", pointFileName(), "fread");     
    return len; 
  } 
  void seek(int offs); 
