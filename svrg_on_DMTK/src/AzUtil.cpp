...
/*-------------------------------------------------------------*/
void AzFile::seekReadBytes(int offs, int len, void *buff) 
{
  const char *eyec = "AzFile::seekRead"; 
  /*-----  seek  -----*/
  if (offs >= 0) {
    if (fseek(fp, offs, SEEK_SET) != 0) {
      throw new AzException(AzFileIOError, eyec, pointFileName(), "seek"); 
...
/*-------------------------------------------------------------*/
void AzFile::seek(int offs) 
{
  seekReadBytes(offs, 0, NULL); 
}

/*-------------------------------------------------------------*/
int AzFile::writeBytes(const void *buff, 
                            int len) 
{
  int io_len = 0; 
  if (len > 0) {
    if (fwrite(buff, len, 1, fp) != 1) {
      throw new AzException(AzFileIOError, "AzFile::writeBytes", pointFileName(), "fwrite");
    }
    io_len = len; 
  }
...
int AzFile::writeBinMarker() 
{
  int int_val = AZ_BIN_MARKER_INT; 
  int len = writeInt(int_val); 
  double dbl_val = AZ_BIN_MARKER_DBL; 
  len += writeDouble(dbl_val); 
  return len; 
}
                  ...
                  int max_line_num) /* -1: means don't care; added 06/15/2013 */
{
  ...
  for ( ; ; ) {
    if (do_exit) break; 
    int len = file.readBytes(buff, buff_size); 
    if (len <= 0) break; 
...
/*-------------------------------------------------------------*/
int AzIIFarr::write(AzFile *file) 
{
  int len = 0; 

  len += file->writeInt((int)ent_type); 
  len += file->writeInt(ent_num); 
  int ex; 
  for (ex = 0; ex < ent_num; ++ex) {
    const AzIIFarrEnt *ep = &ent[ex]; 
    if (ent_type == AzIIFarr_II) {
      len += file->writeInt(ep->int1); 
      len += file->writeInt(ep->int2); 
    }
    else if (ent_type == AzIIFarr_IF) {
      len += file->writeInt(ep->int1); 
      len += file->writeDouble(ep->val); 
    }
    else if (ent_type == AzIIFarr_IIF) {
      len += file->writeInt(ep->int1); 
      len += file->writeInt(ep->int2); 
...
/*-------------------------------------------------------------*/
int AzBytArr::write(AzFile *file) 
const
{
  int io_len = 0; 

  io_len += file->writeInt(len); 

  if (len > 0) {
    if (bytes_long != NULL) {
      io_len += file->writeBytes(bytes_long, len + 1); 
    }
    else {
      io_len += file->writeBytes(bytes_short, len + 1); 
    }
  }
  return io_len; 
}
